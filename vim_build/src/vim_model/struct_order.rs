use crate::vim_model::{Error, Result, Struct};
use indexmap::IndexMap;
use std::cell::RefCell;
use std::collections::HashMap;

pub fn reorder_structs(
    structs: &mut IndexMap<String, RefCell<Struct>>,
) -> Result<IndexMap<String, RefCell<Struct>>> {
    let (ordered, last_child) = generate_last_child_map(structs, "Any");
    let structs = order_structs(structs, &ordered)?;
    populate_last_child(&structs, &last_child)?;
    Ok(structs)
}

fn populate_last_child(
    ...
    if structs.len() != last_child.len() {
        return Err(Error::InternalProcessing(
            "Last child map does not match struct count".to_string(),
        ));
    }
...
fn order_structs(
    ...
    if structs.len() != order.len() {
        return Err(Error::InternalProcessing(
            "Order list length does not match the struct count".to_string(),
        ));
    }
    let mut ordered_structs = IndexMap::new();
    for name in order {
        let (key, value) = structs

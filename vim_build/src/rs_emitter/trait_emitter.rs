...
impl<'a> TraitEmitter<'a> {
    ...
    pub fn emit_trait(&mut self) -> super::Result<()> {
        let vim_type = self.model.structs.get(&self.type_name).ok_or_else(|| Error::TypeNotFound(self.type_name.clone()))?.borrow();
        let type_ref = vim_type.deref();

        if !type_ref.has_children() { 
            return Err(Error::InternalError(format!("Attempt to generate trait for object with no descendants: {}",self.type_name))); 
        };
    ...
    fn emit_trait_type(&mut self, vim_type: &Struct) -> Result<()> {
        if crate::rs_emitter::structs::ANY == self.type_name { return Ok(()); } // Skip the Any type
    ...
    fn emit_trait_deserialization(&mut self) -> Result<()> {
        ...
        self.printer.println(&format!("struct {struct_name}Visitor;"))?;
        self.printer.newline()?;
        self.printer.println(&format!("impl<'de> de::Visitor<'de> for {struct_name}Visitor {{"))?;
        self.printer.indent();
        self.printer.println(&format!("type Value = Box<dyn {struct_name}Trait>;"))?;
        self.printer.newline()?;
        ...
        self.printer.println("{")?;
        self.printer.indent();
        self.printer.println("let deserializer = de::value::MapAccessDeserializer::new(&mut map);")?;
        self.printer.println("let any: VimAny = de::Deserialize::deserialize(deserializer)?;")?;
        self.printer.println("match any {")?;
        self.printer.indent();
        self.printer.println("VimAny::Object(obj) => Ok(CastFrom::from_box(obj)")?;
        self.printer.indent();
        self.printer.println(".map_err(|_| de::Error::custom(\"Internal error converting to trait type\"))?)," )?;
        self.printer.dedent();
        self.printer.println("VimAny::Value(value) => Err(de::Error::custom(format!(")?;
        self.printer.indent();
        self.printer.println(r#""expected object not wrapped value: {:?}","#)?;
    ...
    fn generate_cast_from_trait(&mut self) -> Result<()> {
        if None == self.model.structs.get(&self.type_name) {
            return Err(Error::TypeNotFound(self.type_name.clone()));
        }
        self.printer.println(&format!("impl<From: VimObjectTrait + ?Sized + 'static> CastFrom<From> for dyn {}Trait {{", to_type_name(&self.type_name)))?;
        self.printer.indent();
        self.printer.println("fn from_ref<'a>(from: &'a From) -> Option<&'a Self> {")?;
        self.printer.indent();
        ...
        self.printer.println("}")?;
        self.printer.println("")?;
        self.printer.println("fn from_box(from: Box<From>) -> Result<Box<Self>, Box<dyn std::any::Any + 'static>> {")?;
        self.printer.indent();
        self.printer.println("let data_type = from.data_type();")?;
        self.printer.println("match data_type {")?;
        self.printer.indent();
        for child_struct in self.model.children(&self.type_name)? {
            let type_name = child_struct.borrow().rust_name();
            self.printer.println(&format!("StructType::{type_name} => Ok(from.as_any_box().downcast::<{type_name}>()?),"))?;
        }

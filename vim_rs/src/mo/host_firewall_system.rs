...
impl HostFirewallSystem {
    ...
    pub async fn available_field(&self) -> Result<Option<Vec<crate::types::structs::CustomFieldDef>>> {
        let pv_opt = self.client.fetch_property_raw("", "HostFirewallSystem", &self.mo_id, "availableField").await?;
        match pv_opt {
    ...
    pub async fn firewall_info(&self) -> Result<Option<crate::types::structs::HostFirewallInfo>> {
        let pv_opt = self.client.fetch_property_raw("", "HostFirewallSystem", &self.mo_id, "firewallInfo").await?;
        match pv_opt {
            Some(pv) => Ok(Some(crate::core::client::extract_property(pv)?)),
            None => Ok(None),
        }
    ...
    pub async fn value(&self) -> Result<Option<Vec<Box<dyn crate::types::traits::CustomFieldValueTrait>>>> {
        let pv_opt = self.client.fetch_property_raw("", "HostFirewallSystem", &self.mo_id, "value").await?;
        match pv_opt {
            Some(pv) => Ok(Some(crate::core::client::extract_property(pv)?)),
            None => Ok(None),
        }
...
impl<'b, 'a> miniserde::ser::Map for DisableRulesetRequestTypeSer<'b, 'a> {
...
impl<'b, 'a> miniserde::ser::Map for EnableRulesetRequestTypeSer<'b, 'a> {
    fn next(&mut self) -> Option<(std::borrow::Cow<'_, str>, &dyn miniserde::Serialize)> {
        let seq = self.seq;
        self.seq += 1;
        match seq {
...
impl<'b, 'a> miniserde::ser::Map for UpdateDefaultPolicyRequestTypeSer<'b, 'a> {
    fn next(&mut self) -> Option<(std::borrow::Cow<'_, str>, &dyn miniserde::Serialize)> {
        let seq = self.seq;
        self.seq += 1;
        match seq {
...
impl<'b, 'a> miniserde::ser::Map for UpdateRulesetRequestTypeSer<'b, 'a> {
    fn next(&mut self) -> Option<(std::borrow::Cow<'_, str>, &dyn miniserde::Serialize)> {
        let seq = self.seq;
        self.seq += 1;
        match seq {

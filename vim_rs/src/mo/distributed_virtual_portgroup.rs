...
impl DistributedVirtualPortgroup {
    ...
    pub async fn config(&self) -> Result<crate::types::structs::DvPortgroupConfigInfo> {
        ...
        let bytes = self.client.execute_bytes(req).await?;
        let text = std::str::from_utf8(bytes.as_ref()).map_err(|e| crate::core::client::VimError::ParseError(e.to_string()))?;
        let result: crate::types::structs::DvPortgroupConfigInfo = miniserde::json::from_str(text).map_err(|_| crate::core::client::VimError::ParseError("miniserde deserialization failed".to_string()))?;
        Ok(result)
    }
...
impl miniserde::ser::Map for ReconfigureDvPortgroupRequestTypeSer<'_, '_> {
...
impl miniserde::ser::Map for DvPortgroupRollbackRequestTypeSer<'_, '_> {
    fn next(&mut self) -> Option<(std::borrow::Cow<'_, str>, &dyn miniserde::Serialize)> {
        loop {
            let seq = self.seq;
            self.seq += 1;

...
- **SOAP/XML (`xml` feature) and `govc vcsim` wire compatibility:**
  - **XML Schema instance `type` by namespace URI, not prefix:** deserialization resolves the schema-instance namespace (`http://www.w3.org/2001/XMLSchema-instance`) so any bound prefix works for the typed discriminator (including vcsim’s `_XMLSchema-instance:type`, not only `xsi:type`). Attributes named `type` on other namespaces are ignored. SOAP envelope parsing now uses `quick_xml::NsReader` for namespace-aware scanning.
  - **`vcsim_compat` Cargo feature** (implies `xml`): opt-in tolerant XML path for producers that omit required child elements (notably vcsim’s `HostConfigInfo.optionDef` without `optionType`). `DeserializeOptions`, `from_xml_with`, `with_options`, and `vim_response_with` expose the same semantics explicitly; when the feature is enabled, client-internal SOAP unmarshalling uses tolerant helpers so malformed elements can be dropped instead of failing the whole document or `<returnval>` stream.
  - **`vim_rs::core::client::is_request_canceled_error`:** identifies `RequestCanceled` `MethodFault` values so cache/monitor logic can treat cancel wake-ups consistently across transports.
  - **Offline regression tests** (`vim_rs/tests/vcsim_xml_regression.rs`, fixture `vcsim.xml`) covering vcsim-style prefixes, `WaitForUpdatesEx` captures, and strict vs tolerant `OptionDef` parsing.
- **`ObjectRetriever::retrieve_object`:** single-object `PropertyCollector` fetch without creating a view (avoids view lifecycle for one-off lookups).
- **`CacheManager::set_cancel_wait_on_filter_change`:** when set to **`true`**, `add_cache`, `remove_cache`, and `destroy` issue a best-effort `CancelWaitForUpdates` after mutating filters so an in-flight `WaitForUpdatesEx` returns promptly. Real vCenter/ESXi usually push new filters into a running wait; **vcsim** often does not, so apps that add filters at runtime can appear stuck until the long poll ends. Defaults to **`false`** to preserve prior behavior; enable for vcsim-driven sessions.
- **`Monitor::cancel_wait`** and **`Monitor::reset_version`:** explicit cancel of the session wait and client-side version reset helpers used together with the cancel-on-filter-change policy.
- **`Monitor::wait_updates`:** documents and implements cancel recovery for absorbed `RequestCanceled` faults (clear version, re-arm within the caller’s `delay_s` budget, bounded retry count) so a deliberate wake (filter change or `cancel_wait`) can surface vcsim’s initial enter sets via the empty-version snapshot path.
...
- **SOAP/XML error mapping:** HTTP failures on SOAP transport now classify faults using the typed element inside `<detail>` when present (e.g. `RequestCanceled`), instead of relying on `<faultstring>` alone—important for **vcsim**, which often returns an empty `faultstring` on cancel. Envelope-level faults without a typed API fault still surface as parse/string errors rather than a synthetic `MethodFault`.
- **`ObjectRetriever` / `CacheManager` view teardown:** cached views store the full `ManagedObjectReference` and `DestroyView` is invoked with the correct managed-object **type** string via `VimClient::invoke_void`, replacing use of the `View` stub typed only as `View` (which could send the wrong type for list vs container views).

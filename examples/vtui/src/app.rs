...
impl App {
    pub fn new(event_handler: EventHandler, cache_mgr: Rc<RefCell<CacheManager>>, vms: VmListWidget) -> Self {
        Self {
            should_quit: false,
            cache_mgr,
            vms,
    ...
    async fn handle_app_event(&mut self, event: AppEvent) -> anyhow::Result<()> {
        match event {
            AppEvent::PropertyCollector(update) => self.cache_mgr.borrow_mut().apply_updates(update)?,

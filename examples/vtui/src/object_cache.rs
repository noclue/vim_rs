...
use vim_rs::types::structs::{ManagedObjectReference, ObjectSpec, ObjectUpdate, PropertyFilterUpdate, PropertySpec, TraversalSpec, WaitOptions};
use vim_rs::core::pc_helpers::{Error, Result};
use vim_rs::types::enums::{MoTypesEnum, ObjectUpdateKindEnum};
...
pub trait Cacheable: TryFrom<ObjectUpdate, Error: Display> {
    ...
    fn apply_update(&mut self, update: ObjectUpdate) -> vim_rs::core::pc_helpers::Result<()>;
...
impl CacheManager {
    ...
    pub async fn add_cache(&mut self, cache: Box<dyn Cache>, object_set: Vec<ObjectSpec>) -> Result<ManagedObjectReference> {
        let filter_spec = vim_rs::types::structs::PropertyFilterSpec {
            object_set,
...
impl Monitor {
    ...
    pub async fn wait_updates(&mut self, delay_s: i32) -> anyhow::Result<Option<Vec<PropertyFilterUpdate>>>
    {

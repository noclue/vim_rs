...
use crate::types::structs::HostAccountSpecTrait;
...
impl HostLocalAccountManager {
    ...
    pub async fn create_group(&self, group: &dyn HostAccountSpecTrait) -> Result<()> {
    ...
    /// invalid format.
    pub async fn create_user(&self, user: &dyn HostAccountSpecTrait) -> Result<()> {
    ...
    pub async fn update_user(&self, user: &dyn HostAccountSpecTrait) -> Result<()> {
...
struct CreateGroupRequestType<'a> {
    group: &'a dyn HostAccountSpecTrait,
}
#[derive(serde::Serialize)]
#[serde(tag="_typeName")]
struct CreateUserRequestType<'a> {
    user: &'a dyn HostAccountSpecTrait,
}
#[derive(serde::Serialize)]
#[serde(tag="_typeName")]

...
use crate::types::structs::NodeDeploymentSpecTrait;
...
impl FailoverClusterConfigurator {
    ...
    /// 
    /// Refers instance of *Task*.
    pub async fn create_witness_node_task(&self, witness_deployment_spec: &dyn NodeDeploymentSpecTrait, source_vc_spec: &SourceNodeSpec) -> Result<ManagedObjectReference> {
...
struct CreateWitnessNodeRequestType<'a> {
    ...
    witness_deployment_spec: &'a dyn NodeDeploymentSpecTrait,
    #[serde(rename = "sourceVcSpec")]
    source_vc_spec: &'a SourceNodeSpec,
}
#[derive(serde::Serialize)]
